...
import {Skill, SPRITE_SHEET} from './Skills';
...
export class Game {
  ...
  rocket = false;
  atlantis = false;
  punchSound = new Audio('/flintstones/sounds/punch.mp3');
  woodenHitSound = new Audio('/flintstones/sounds/wooden_crash.mp3');
  whooshSound = new Audio('/flintstones/sounds/whoosh_1.mp3');
  heartBeatSound = new Audio('/flintstones/sounds/heart_beat.mp3');
  starHitSound = new Audio('/flintstones/sounds/ckeck_1.mp3');
  bellTransitionSound = new Audio('/flintstones/sounds/bell_transition.mp3');
  hitSound = new Audio('/flintstones/sounds/hit.mp3');
  jumpSound = new Audio('/flintstones/sounds/launch.mp3');
  ...
  update(deltaTime: number) {
    ...
    this.fireballs = this.fireballs.filter((ball) => {
      ...
      if (this.isPlay) ball.sound.play();
      ball.update();
    ...
    this.enemies = this.enemies.filter((enemy) => {
      if (this.isPlay) enemy.sound.play();
  ...
  addPerk(x: number, y: number) {
    ...
    else if (this.mac && !this.program1) {
      this.perks.push(new Program(this, x, y));
    } else if (this.program1 && !this.graduate) {
  ...
  addSkill() {
    return Object.keys(this.ganeSkills).slice(0, ++this.mySkill);
  }
