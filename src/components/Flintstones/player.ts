...
export class Player {
  ...
  constructor(public game: Game, public deltaTime: number) {
    this.image.src = '/flintstones/images/player.png';
  ...
  collisionWithStars() {
    this.game.stars = this.game.stars.filter((star) => {
      ...
      if (collision) {
        ...
        if (this.game.isPlay) this.game.starHitSound.play();
      ...
      if (
        ...
        if (this.killTime < 0) {
          ...
          if (enemy.lives > 0) {
            ...
            if (this.game.isPlay) this.game.punchSound.play();
        ...
        this.hitTimer = 0;
        if (this.game.isPlay) this.game.hitSound.play();
  ...
  collisionWithPerks() {
    this.game.perks.forEach((perk) => {
      ...
      if ((collision || collisionHead) && perk.ready) {
        if (this.game.isPlay) this.game.bellTransitionSound.play();

        if (perk.name === PerksE.piggy && !this.game.piggy) {
          this.game.piggy = true;
        }
        if (perk.name === PerksE.program && !this.game.program1) {
          this.game.program1 = true;
          this.game.program.push(new ShowProgram(this.game, PerksE.program));
        }
        ...
        if (perk.name === PerksE.perfolenta) {
          ...
          this.game.program.push(new ShowProgram(this.game, PerksE.perfolenta));
        }
        if (perk.name === PerksE.radiolamp) {
          this.game.radiolamp = true;
          this.game.program.push(new ShowProgram(this.game, PerksE.radiolamp));
        }
        if (perk.name === PerksE.teamwin) {
          this.game.teamwin = true;
          this.game.program.push(new ShowProgram(this.game, PerksE.teamwin));
        }
        if (perk.name === PerksE.lochman) {
          this.game.lochman = true;
          this.game.program.push(new ShowProgram(this.game, PerksE.lochman));
        }
        if (perk.name === PerksE.rocket) {
          this.game.rocket = true;
          this.game.program.push(new ShowProgram(this.game, PerksE.rocket));
        }
        if (perk.name === PerksE.atlantis) {
          this.game.atlantis = true;
          this.game.program.push(new ShowProgram(this.game, PerksE.atlantis));
        }
  ...
  collisionWithBarrels(deltaTime: number) {
    ...
    this.game.barrels.forEach((barrel) => {
      ...
      if (
        ...
        if (this.game.isPlay) this.game.woodenHitSound.play();
